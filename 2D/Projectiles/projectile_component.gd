extends Node2D # This is only a 2D node because the VisibleOnScreenNotifier2D
class_name ProjectileComponent

signal free_projectile

@export var root: Node2D ## projectile root node
#@export var velocity_component : VelocityComponent
#@export var stats_component : StatsComponent

@export var direction : Vector2 ## direction of projectile
@export_category("Stats")
@export var damage : int = 1 ## damage of projectile, could be part of the stats component just would have to make one for specifically projectiles in this case.
...
@export var random_direction : bool = false ## should projectile go in random direction on ready, used for debug mainly
